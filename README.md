...
2. フォーム送信を JavaScript で書き換え
  ...
  + `fetch("/contact", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(data), })` で送信
...
1. `functions/contact.ts` にチェックを追加
...
4. `functions/contact.ts` で token を検証
