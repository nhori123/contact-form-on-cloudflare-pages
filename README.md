...
2. Cloudflare Pages の環境変数に設定
  + `RESEND_API_KEY`、`MAIL_TO`、`MAIL_FROM`シークレットとして追加
...
> 環境変数一覧（参考）
> RESEND_API_KEY=xxxx
> MAIL_TO=xxxx@example.com  // RESENDの登録に利用したアドレス
> MAIL_FROM=onboarding@resend.dev // RESENDの共有アドレス

> 本番ではRESENDに独自ドメインを登録し、`MAIL_FROM`は独自ドメインのメールアドレスを利用するようにしてください。
> また、独自ドメインを登録することでMAIL_TOに任意のメールアドレスを指定できるようになります。
